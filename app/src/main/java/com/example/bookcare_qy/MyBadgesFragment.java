...
public class MyBadgesFragment extends Fragment {
    ...
    private void loadUserPoints(TextView tvCurrentLevel, TextView tvLevelNumber, TextView tvTotalPoints, 
        ...
        userRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (user != null) {
                    // Calculate total points: same as EcoPointsFragment
                    int booksExchanged = user.getBooksExchanged();
                    int booksDonated = user.getBooksDonated();
                    ...
                    currentUserPoints = pointsFromExchanges + pointsFromDonations;
                    
                    // Update badge level (50 points per level, starting at level 1)
                    // Level 1 = 0-49 points, Level 2 = 50-99 points, etc.
                    int badgeLevel = (currentUserPoints / 50) + 1;
                    if (currentUserPoints == 0) badgeLevel = 0; // Special case: 0 points = no level yet
                    if (badgeLevel > 50) badgeLevel = 50; // Max level is 50
    ...
    private void initializeBadges() {
        for (int i = 1; i <= 50; i++) { // Levels 1 to 50 (50 points per level, starting at level 1)
            allBadges.add(new Badge("Level " + i, "", (i - 1) * 50, i));
        }
    }
    ...
    private void calculateAndDisplayProgression(TextView tvCurrentLevel, TextView tvLevelNumber, TextView tvTotalPoints, ProgressBar pbLevelProgress, TextView tvProgressText, TextView tvBadgesHeader) {
        // Badge level: 50 points per level, starting at level 1
        // Level 1 = 0-49 points, Level 2 = 50-99 points, etc.
        int userLevel;
        if (currentUserPoints == 0) {
            userLevel = 0; // No level yet
        } else {
            userLevel = (currentUserPoints / 50) + 1;
            if (userLevel > 50) userLevel = 50; // Max level is 50
        }
        
        int pointsForCurrentLevel = userLevel > 0 ? (userLevel - 1) * 50 : 0;
        int pointsForNextLevel = userLevel * 50;
        int progress = currentUserPoints - pointsForCurrentLevel;

        if (userLevel == 0) {
            tvCurrentLevel.setText("No Level Yet");
            tvLevelNumber.setText("No Level Yet");
            pbLevelProgress.setMax(50);
            pbLevelProgress.setProgress(currentUserPoints);
            tvProgressText.setText(currentUserPoints + " / 50 points to Level 1");
        } else {
